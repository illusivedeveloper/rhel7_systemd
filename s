...
Similar thing can bve done with rhel7_init ubi image for non OCI-systemd-hooks. Run image using following command:
	$ docker container run -d -P --tmpfs /run --tmpfs /tmp -v /sys/fs/cgroup:/sys/fs/cgroup rhel7_init
...
Another way using Privileged mode : $ docker container run -d --privileged -v /sys/fs/cgroup:/sys/fs/cgroup rhel7_init
